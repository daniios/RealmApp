...
final class TasksViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let doneActionTitle = task.isComplete ? "Mark Undone" : "Mark Done"
        let doneAction = UIContextualAction(style: .normal, title: doneActionTitle) { [unowned self] _, _, isDone in
            if task.isComplete {
                storageManager.markTaskAsUndone(task)
                tableView.moveRow(at: indexPath, to: IndexPath(row: 0, section: 0))
            } else {
                storageManager.markTaskAsDone(task)
           ...
           alertBuilder
               ...
               .addAction(
                   ...
                   if let task, let completion {
                       self?.storageManager.edit(task, newTask: taskTitle, newNote: taskNote)
                       completion()
                       return
                   }
